...
class SkipGram(object):
	...
	def train(self, filename=None):
		...
		for i in range(8):
		  	print(self.batch[i], self.reverse_dictionary[self.batch[i]],'->', self.labels[i, 0], self.reverse_dictionary[self.labels[i, 0]])
		...
		with tf.Session(graph=graph) as session:
		    ...
		    #    print(log_str)'''
		  	...
		  	clustered_synonyms = self.cluster(final_embeddings)
	...
	def cluster(self, final_embeddings):
		...
		tsne = TSNE(perplexity=30, n_components=2, init='pca', n_iter=5000)
  		...
  		#just to make sure this is a nx2 matrix
  		print(np.array(low_dim_embs).shape()) 
  		clustered_synonyms = KMeans(n_clusters=2, random_state=0).fit(low_dim_embs)
